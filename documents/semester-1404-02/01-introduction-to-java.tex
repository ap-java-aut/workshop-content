...
\lr{JDK} ی که قبل‌تر دانلود کرده‌اید انتخاب شده باشد. من نسخهٔ
\lr{\texttt{25.0.2}} را نصب کرده‌ام و برای این پروژه نیز از همان نسخه
استفاده کرده‌ام:
...
\lr{\texttt{println}} علاوه بر رشته‌ها \lr{(String)} ، می‌تواند انواع دادهٔ
دیگری مانند \lr{\texttt{int}} و \lr{\texttt{boolean}} را نیز چاپ کند.
بنابراین می‌توانید با همان دستور، مقادیر متنوعی را در خروجی چاپ کنید:
...
انواع داده‌ای مانند \lr{\texttt{int}} و \lr{\texttt{boolean}} و سایر
نوع‌ها اشاره شد، بیایید نخستین متغیرهای خود را در جاوا تعریف کنیم.
...
\lr{\texttt{int}} ، \lr{\texttt{boolean}} ، \lr{\texttt{float}} و انواع
مشابه آن‌ها، \lr{data type} گفته می‌شود. \lr{data} type‌ های اصلی جاوا که
به آنها \lr{Primitive data types} هم گفته می‌شود، به همراه رنجی که پوشش
می‌دهند، در جدول زیر آمده‌اند. همانند زبان \lr{C} ، نباید مقداری خارج از
رنج تعریف‌شده برای هر \lr{type} به آن اختصاص دهید.
...
می‌توانید به‌صورت مشابه، متغیرهایی از هر یک از این \lr{type} ها تعریف کرده
و آن‌ها را چاپ کنید:
...
در \lr{java} ، یک \lr{\texttt{type}} برای متغیرهاتون به اسم
\lr{\texttt{String}} هم دارید، که البته جزو \lr{primitive data type} ها
...
می‌توانید هنگام تعریف متغیر، آن را مقداردهی نکنید و صرفا به نوشتن
\lr{type} شون اکتفا کنید:
...
در مثال بالا، چون مقادیر \lr{\texttt{10}} و \lr{\texttt{20}} به
\lr{\texttt{a}} و \lr{\texttt{b}} داده شده‌اند، کامپایلر تشخیص می‌دهد که
این دو متغیر از نوع \lr{\texttt{int}} هستند و مطابق آن رفتار می‌کند. توجه
داشته باشید که \lr{\texttt{var}} یک \lr{data type} جدید نیست، بلکه صرفاً
روشی برای ساده‌تر کردن نوشتن کد است. لازم نیست برای همه متغیرها،
\lr{type} های آن‌ها را بنویسید. کامپایلر نوع متغیرها را براساس مقدار
...
متغیر \lr{\texttt{long}} استفاده کرده‌اید که از نظر ظرفیت باید توان
نگه‌داری آن را داشته باشد همچنان با خطای \lr{number too large} مواجه
می‌شوید. علت چیست؟ خطی را که در آن \lr{a} را تعریف کرده‌اید، با خط زیر
جایگزین کنید:
...
مشکل برطرف شد، درست است؟ علت خطای قبلی، نوع متغیر \lr{\texttt{a}} نبود؛
زیرا \lr{\texttt{long}} قادر است اعداد بسیار بزرگ‌تری را نیز در خود ذخیره
کند. مسئله این بود که خود جاوا نمی‌دانست با عدد $99999999999$ چگونه
برخورد کند: آیا باید آن را \lr{int} در نظر بگیرد، \lr{\texttt{long}}
بداند یا حتی \lr{\texttt{double}}؟ در نسخهٔ دوم کد، با گذاشتن
\lr{\texttt{L}} در انتهای عدد، به جاوا گفتید که این مقدار از نوع
\lr{\texttt{long}} است و باید همانند یک \lr{\texttt{long}} با آن رفتار
شود.
...
\lr{\texttt{'\textbackslash n'}} و \lr{\texttt{3.14}} هستند. همان‌طور که
قبل‌تر اشاره شد، \lr{literal} ها نیز مانند متغیرها دارای \lr{type} هستند.
برای مثال، عدد \lr{\texttt{123}} از نوع \lr{\texttt{int}} محسوب می‌شود،
در حالی که \lr{\texttt{123L}} از نوع \lr{\texttt{long}} است. همچنین اگر
عددی مانند \lr{\texttt{3.14}} را به‌صورت اعشاری بنویسید، جاوا به‌طور
پیش‌فرض آن را از نوع \lr{\texttt{double}} در نظر می‌گیرد؛ و اگر بخواهید
نوع آن \lr{\texttt{float}} باشد، باید از \lr{\texttt{3.14F}} استفاده
کنید. افزون بر این، می‌توانید برای اعداد اعشاری از نمایش علمی
\lr{(scientific notation)} نیز بهره ببرید:
...
کلیدواژهٔ \lr{\texttt{final}} این امکان را به شما می‌دهد که یک
\lr{constant} تعریف کنید. \lr{constant} ها پس از تعریف و مقداردهی، هرگز
مقدارشان تغییر نمی‌کند.
...
فقط توجه داشته باشید که \lr{\texttt{String}} ها را با \lr{\texttt{==}}
مقایسه نکنید. اگر این کار را انجام دهید، خود \lr{IntelliJ} به شما یک
\lr{warning} می‌دهد و پیشنهاد می‌کند از متد \lr{\texttt{equals}} استفاده
کنید:
...
شیوهٔ صحیح مقایسهٔ \lr{\texttt{String}} ها در جاوا به این صورت است:
...
دلیل این موضوع آن است که همان‌طور که قبل‌تر گفتیم، \lr{\texttt{String}} یک
...
شاید برایتان جالب باشد بدانید که برای تعریف یک \lr{block} ، الزاماً نیازی
به استفاده از \lr{\texttt{if}} ،\lr{\texttt{for}} یا ساختارهای مشابه
نیست و می‌توانید بدون آن‌ها نیز یک \lr{block} ایجاد کنید:
...
این خطا به این معناست که نمی‌توانید از متغیرهای محلی‌ای که داخل
\lr{\texttt{if}} ،\lr{\texttt{for}} ، متدها یا هر ساختار بلوکی دیگری
تعریف شده‌اند، در بیرون از همان \lr{block} استفاده کنید.
...
پس از انجام این کار، باید در کنار کدهایتان دایرکتوری‌ای به نام
\lr{\texttt{out}} ایجاد شده باشد که شامل نسخهٔ کامپایل‌شدهٔ برنامهٔ شماست:
...
روی \lr{\texttt{out}} کلیک کنید و سپس وارد \lr{production} شوید تا
دایرکتوری‌ای با نام پروژهٔ خود مشاهده کنید؛ آن را نیز باز کنید.
