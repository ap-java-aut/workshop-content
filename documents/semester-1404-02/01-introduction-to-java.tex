...
\footnote{در این‌جا نیز لازم نیست با \lr{\texttt{public static}} که پیش
  از \lr{\texttt{MAX\_AGE}} آمده کاری داشته باشید؛ در داکیومنت‌های بعدی
  به‌طور کامل با آن‌ها آشنا می‌شویم و از آن‌ها استفاده خواهیم کرد.}
...
همچنین همواره سعی کنید به warningها، hintها و به‌طور کلی ابزارهایی که
\lr{IntelliJ} در اختیارتان قرار می‌دهد توجه کنید. با استفاده از آن‌ها
می‌توانید کد خود را بهبود دهید و نکات ارزشمندی بیاموزید.
...
در کد زیر، یک حلقهٔ \lr{for} صدتایی می‌نویسیم:
...
این‌جا نیز می‌توانید انجام دهید. می‌توانید به‌جای \lr{i++} هر عبارت مشابه
دیگری که تغییری در \lr{i} ایجاد می‌کند قرار دهید. هر دو \lr{\texttt{for}}
زیر صحیح‌اند؛ آن‌ها را اجرا کنید و خروجی‌شان را بررسی کنید:
...
مانند C، در این‌جا نیز می‌توانید از \lr{\texttt{break}} و
