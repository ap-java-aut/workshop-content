...
- کلاس `ArrayList` در جاوا چیست، چه تفاوت‌هایی با آرایه‌ی ساده دارد، و متدهای اصلیِ آن (مانند `add` ، `remove` ، و...)  چیست.
- کلاس `String` در جاوا برای ذخیره‌ی رشته‌ها چطور کار می‌کند و برخی از متدهای آن مثل `charAt` ، `equals` ، و... چطور می‌توانند به ما کمک کنند.
- کلاس `HashMap` در جاوا چیست، و key و value چه معنایی دارند. همچنین باید با متدهای اصلی این کلاس مثل `get` و `put` آشنا باشید.
- حلقه‌های enhanced for در جاوا چه کاربردی دارند و بتوانید از آن برای پیمایش یک آرایه یا هش‌مپ استفاده کنید.
