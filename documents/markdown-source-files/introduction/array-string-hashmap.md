...
مقادیر دیفالت typeهای عددی مثل `int`، `float` و غیره در جاوا صفر است. مقدار دیفالت `boolean` همان `false` بوده و مقدار دیفالت `char` هم `‘\n’` می‌باشد؛ اما مقدار دیفالت reference type ها همیشه `null` است که در مباحث مربوط به OOP راجع به آن‌ها مطالعه خواهید کرد.
...
ما، طول یک آرایه را با استفاده از `numbers.length` بدست می‌آوریم. به متغیرهایی مثل `length`یا  `variable` در جاوا field های یک کلاس می‌گویند که از متغیرهای مربوط به آن کلاس می‌باشد. با این متغیرها و انواع آن‌ها وقتی OOP را یاد گرفتید بیشتر آشنا خواهید شد. تا آن زمان، بیاید چندتا از متدها و فیلدهای پرکاربرد آرایه را بررسی کنیم:
...
متد `length` طول آرایه را به شما نشان می‌دهد:
...
**تعریف آرایه دو بعدی و چند بعدی بدون مقدار دهی اولیه:** مثل قبل، می‌توانید بدون مقداردهی اولیه آرایه تعریف کنید و باز هم خانه‌های آرایه‌ها با مقدار دیفالت type شان پر می‌شوند.
...
### reference type های معادل primitive type ها در جاوا

برای هر primitive type توی جاوا، مثل `int`، `boolean`، `char` و امثال آن‌ها، یک معادل از جنس reference type وجود دارد. وقتی که با مفهوم object ها بیشتر آشنا بشویم، این موضوع را مجددا بررسی می‌کنیم، ولی چون برای `ArrayList` به آن‌ها نیاز داریم، لازم است که یک نگاه مختصر به آن‌ها بیندازیم.
...
تنها تفاوت این دو، این است که `Integer`، reference type ای است که معادل `int` می‌باشد. کامپایلر شما حتی به شما این اجازه را می‌دهد که با متغیری از جنس `Integer` مثل یک `int` برخورد کنید:
...
در مورد این که چرا وقتی `int` را داریم، به `Integer` هم نیاز داریم بعدا توضیح خواهیم داد. برای الآن کافی است بدانید که این reference type ها وجود دارند و تا حد خیلی خوبی هم معادل همتای primitive type شان هستند. فهرست کامل آن‌ها در جدول زیر آمده است:
...
اگر کلیدی در HashMap وجود نداشته باشد و ما آن را به عنوان ورودی به متد `get` بدهیم، خروجی این متد برای ما `null` است. `null` ، مقدار خاصی برای reference type هاست که نشان می‌دهد که آن‌ها هنوز وجود ندارند و با آن در OOP بیشتر آشنا می‌شویم:
...
-  پیدا کردن سریع داده‌ها: میتوانید با دادن `key` خیلی سریع `value` آن را پیدا کنید. در درس ساختمان داده خواهید دید که این کار، بسیار سریع تر از گشتن در یک آرایه است:
...
-  شمارش تعداد دفعات تکرار کلمات: با هر بار دیدن کلمه در متن `value` تغییر داده شود و یک عدد به آن اضافه شود.
...
-  نگهداری ارتباط بین دو مجموعه داده: برای مثال `key` نام دانش‌آموز و `value` مجموعه نمراتش باشد.
-  گروه‌بندی داده‌ها: برای مثال `key` الکترونیک و `value` لیست محصولات الکترونیک (لپ تاپ، تلفن همراه و ...) باشد.
...
در این داکیومنت با چند ابزار خیلی پرکاربرد در جاوا آشنا شدیم که در اکثر تمرین‌ها و پروژه‌ها به کار می‌آیند:

### 1)Array
- آرایه برای ذخیره‌ی چند داده‌ی **هم‌نوع** زیر یک نام استفاده می‌شود.
- آرایه‌ها **zero-based** هستند (اندیس از ۰ شروع می‌شود).
- اندازه‌ی آرایه **ثابت** است و بعد از ساخت قابل تغییر نیست.
- طول آرایه با `length` به‌دست می‌آید.
- با حلقه‌ی `for` یا **enhanced for loop** می‌توان روی آرایه پیمایش کرد.
- با `System.arraycopy` برای کپی کردن و `Arrays.sort` برای مرتب‌سازی آرایه‌ها آشنا شدیم.
- آرایه‌های چندبعدی را مثل ماتریس/حجم دیدیم و نحوه‌ی تعریف، دسترسی و پیمایش آن‌ها را یاد گرفتیم.

### 2) ArrayList
-  شبیه آرایه است ولی **اندازه‌ی پویا** دارد 
- ` فقط **reference type** می‌گیرد، برای اعداد از wrapperها مثل `Integer` و `Float` استفاده کردیم.
- متدهای مهم `ArrayList`:
  - `add` (اضافه کردن عنصر، هم آخر لیست هم در اندیس مشخص)
  - `remove` (حذف با اندیس یا حذف عنصر)
  - `get` (دسترسی به عنصر با اندیس)
  - `set` (تغییر مقدار در اندیس مشخص)
  - `size` (تعداد عناصر لیست)

### 3) String
-  برای نگهداری متن استفاده می‌شود و با `""` تعریف می‌شود.
- با متدهای پرکاربرد `String` آشنا شدیم:
  - `charAt`, `toCharArray`
  - `concat` و همچنین عملگر `+`
  - `contains`, `startsWith`, `endsWith`
  - `equals`, `indexOf`, `length`
  - `split`, `substring`

### 4) HashMap
-  یک ساختار داده‌ی **کلید-مقدار** (Key-Value) است.
- کلیدها **یکتا** هستند و با کلید می‌توان سریع به مقدار رسید.
- متدهای مهم `HashMap`:
  - `put` (اضافه/به‌روزرسانی)
  - `get` (گرفتن مقدار؛ اگر کلید وجود نداشته باشد `null`)
  - `containsKey` (بررسی وجود کلید)
- کاربردهای رایج:
  - دسترسی سریع به اطلاعات (مثل دفترچه تلفن)
  - شمارش تکرارها
  - نگهداری ارتباط بین دو مجموعه داده و گروه‌بندی

### 5) enhanced for loop
- وقتی فقط **مقدار** عناصر مهم است (نه اندیس)، از enhanced for loop استفاده می‌کنیم.
- این حلقه برای پیمایش آرایه‌ها، `ArrayList` و سایر ساختارهای قابل پیمایش خیلی کاربردی است.
