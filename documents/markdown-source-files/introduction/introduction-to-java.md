...
این داکیومنت برای آشنایی اولیه با syntax زبان جاوا طراحی شده‌است. در این مسیر قرار است کمی با آن کدنویسی کنیم و مفاهیم پایه‌ای را بیاموزیم. ممکن است هنگام کار با جاوا، زبان‌هایی مانند C یا C++ را به یاد بیاورید؛ این طبیعی است، زیرا syntax این زبان‌ها شباهت زیادی به یکدیگر دارد. بسیاری از کارهایی که در این‌جا انجام می‌دهیم، مشابه تجربیاتی است که احتمالاً پیش‌تر در C داشته‌اید.
...
صفحه‌ای مشابه تصویر زیر برایتان باز می‌شود. نام پروژه را انتخاب کنید، اطمینان داشته باشید که مسیر ذخیره‌سازی صحیح است و حتماً بررسی کنید همان JDKی که قبل‌تر دانلود کرده‌اید انتخاب شده باشد. من نسخهٔ 25.0.2 را نصب کرده‌ام و برای این پروژه نیز از همان نسخه استفاده کرده‌ام:
...
در این کد، یک کلاس با نام Main می‌بینید که درون آن متدی به نام main قرار دارد. در جلسات آینده توضیح خواهیم داد که کلاس چیست، public چه مفهومی دارد و چگونه می‌توانید کلاس‌های خود را ایجاد کنید. اما در حال حاضر، تمام توابع و کدهایی که می‌نویسید در همین کلاس Main قرار می‌گیرد و نیازی نیست فعلاً درگیر مفاهیم مرتبط با کلاس‌ها شوید.
...
در این class، متدی با نام main مشاهده می‌کنید. در جاوا به توابع، Method گفته می‌شود (توجه داشته باشید که استفاده از واژهٔ Function برای آن‌ها اصطلاح دقیقی نیست). این متد مشابه تابع main در زبان C است که قبل‌تر با آن آشنا شده‌اید و نقطهٔ شروع اجرای برنامه محسوب می‌شود:
...
شاید برایتان سوال شده‌باشد که public static void و String[] args دقیقاً چه هستند. کلیدواژه‌های public و static از مفاهیمی هستند که در مباحث شی‌گرایی دربارهٔ آن‌ها صحبت خواهیم کرد، بنابراین در حال حاضر نیازی به نگرانی در مورد آن‌ها نیست.

اما void احتمالاً از تجربیات پیشین برنامه‌نویسی برایتان آشناست. زمانی که void در تعریف یک متد استفاده می‌شود، به این معناست که آن متد هیچ مقداری را به‌عنوان خروجی باز نمی‌گرداند. در اینجا نیز همین مفهوم برقرار است؛ یعنی متد main هیچ مقداری را return نمی‌کند.
...
این تکه کد برای چاپ `"Hello, World!"` نوشته شده‌است. اگر این خط را حذف کرده و دوباره بنویسید، متوجه خواهید شد که دستور println علاوه بر رشته‌ها (String)، می‌تواند انواع دادهٔ دیگری مانند int و boolean را نیز چاپ کند. بنابراین می‌توانید با همان دستور، مقادیر متنوعی را در خروجی چاپ کنید:
...
```
در java، یک `type` برای متغیرهاتون به اسم `String` هم دارید، که البته جزو primitive data typeها نیست. برای نمونه `Hello, World`ای که در مثال اولتون `print` کردید، نمونه‌ای از همین `type` بود. می‌توانید به شکل زیر یک String را تعریف کنید:
```java
public class Main {
   public static void main(String[] args) {  
...
با این حال، حتماً باید پیش از استفاده از متغیر، به آن مقدار اختصاص دهید؛ در غیر این صورت، جاوا اجازهٔ compile شدن برنامه را نخواهد داد. به عنوان مثال، اگر تلاش کنید تا کد زیر را اجرا کنید:
...
این literalها به‌ترتیب 10، '\n' و 3.14 هستند. همان‌طور که قبل‌تر اشاره شد، literalها نیز مانند متغیرها دارای type هستند. برای مثال، عدد 123 از نوع int محسوب می‌شود، در حالی که 123L از نوع long است. همچنین اگر عددی مانند 3.14 را به‌صورت اعشاری بنویسید، جاوا به‌طور پیش‌فرض آن را از نوع double در نظر می‌گیرد؛ و اگر بخواهید نوع آن float باشد، باید از 3.14F استفاده کنید. افزون بر این، می‌توانید برای اعداد اعشاری از نمایش علمی (scientific notation) نیز بهره ببرید:
...
در کد زیر، یک constant به نام MAX_AGE را در جاوا تعریف کرده‌ایم:
```java
public class Main {
...
در این‌جا نیز لازم نیست با public static که پیش از MAX_AGE آمده کاری داشته باشید؛ در داکیومنت‌های بعدی به‌طور کامل با آن‌ها آشنا می‌شویم و از آن‌ها استفاده خواهیم کرد.
...
همچنین همواره سعی کنید به warningها، hintها و به‌طور کلی ابزارهایی که IntelliJ در اختیارتان قرار می‌دهد توجه کنید. با استفاده از آن‌ها می‌توانید کد خود را بهبود دهید و نکات ارزشمندی بیاموزید.
...
اگر درک do while در حال حاضر کمی دشوار است، نگران نباشید؛ در آینده با کاربردها و شیوهٔ استفادهٔ آن آشنا خواهید شد.

علاوه بر این موارد، برای iterate روی اعضای یک آرایه، لیست و ساختارهای مشابه، نوع خاصی از for وجود دارد که در داکیومنت‌های بعدی با آن آشنا می‌شوید.
...
شاید متوجه نشده باشید، اما در دو کد بالا، ifها آکولاد ندارند. در جاوا، اگر بدنهٔ یک if، for، while، else یا هر ساختار بلوکی دیگری تنها یک خط باشد، نیازی به قرار دادن آکولاد نیست.
...
وظیفهٔ این متد آن است که بررسی کند آیا ایمیل ورودی شامل کاراکتر @ هست یا نه. احتمالاً می‌دانید که در آینده توضیح خواهیم داد چرا از public static استفاده می‌کنیم. فعلاً کافی است بدانید هر چیزی را که در کلاس Main تعریف می‌کنید، باید پیش از استفاده بنویسید.

همان‌طور که مشاهده می‌کنید، تعریف متدها در جاوا از نظر ساختاری بسیار شبیه به Functionها در زبان C است؛ بنابراین هر کاری که در C انجام می‌دادید، این‌جا نیز به همان شکل انجام می‌دهید.
...
به هر چیزی که میان دو آکولاد {} نوشته می‌شود، یک block گفته می‌شود. برای نمونه، کد زیر ۳ block تعریف می‌کند:
```java
public class Main {
    public static void main(String[] args) {
        for (int i = 0; i < 10; i++) {  
...
ورودی گرفتن در جاوا کمی با زبان‌های دیگر تفاوت دارد. برای استفاده از آن، ابتدا باید یک object از نوع Scanner با استفاده از System.in ایجاد کنید. اگر جملهٔ پیشین برایتان نامفهوم بود، نگران نباشید؛ زمانی که شی‌گرایی را بیاموزید، درک آن برایتان ساده‌تر خواهد شد.فعلاً کد زیر را بالای متد main خود قرار دهید:
