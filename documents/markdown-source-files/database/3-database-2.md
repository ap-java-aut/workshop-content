...
به دومین هفته‌ی درس دیتابیس خوش اومدید! این هفته قراره دانش هفته‌ی قبل‌تون رو یه کم گسترش بدین و با کمک دیتابیس todo list‌ تون، چند تا دستور جدید SQL یاد بگیرید.

قبل از اینکه وارد مباحث جدید بشیم، بیاید یه مرور سریع روی هفته‌ی قبل داشته باشیم. توی داکیومنت «دیتابیس مقدماتی ۱» با هم درباره‌ی اهمیت دیتابیس‌ها صحبت کردیم و قبل از اینکه شروع به کار کنید، با مفاهیم اولیه‌ی دیتابیس آشنا شدید و چند تا جدول برای todo list‌ تون ساختید. بعدش رفتیم سراغ کدنویسی و اولین پروژه‌ی SQL‌ تون رو استارت زدید. با کدهایی که هفته‌ی قبل نوشتیم، الان می‌تونین توی دیتابیس‌تون جدول درست کنید، برای ستون‌های مختلفش محدودیت و شرط بذارین و بخش‌هایی از جدول‌تون رو که نیاز دارید، مشاهده کنید.

حالا وقتشه که با todo list‌ تون کارهای بیشتری انجام بدیم. از توابع و دستورات ساده شروع می‌کنیم و کم‌کم می‌ریم سراغ دستورهای پیشرفته‌تر.

## ‏aggregate function ها

قبل از شروع، [این اسکریپت](https://drive.google.com/file/d/1xBOhJ4kz8xDRMn2RxBB9r6Ol2G0830tX/view?usp=sharing) SQL رو دانلود کنید و توی DataGrip اجراش کنید. این اسکریپت، شامل تمام جداول و رکوردهایی هست که توی این داک بهشون نیاز داریم. یه نگاه به دستوراتش بندازید و خودتون ببینید که چه کار می‌کنه.

یه مقدار روی جداول جدید دیتابیس‌تون `select` بزنید که ببینید داده‌هامون به چه صورتی هستند. بعد از این که حدودی با این دیتابیس کار کردین، به خوندن داک ادامه بدید.

### شمارش

به جدول `tasks` نگاهی بیندازید. اگه کار هاتون رو قصد داشته باشید که انجام بدید، احتمالاً میخواهید بدانید که چندتا تسک برای انجام دارید. این‌جاست که دستور `count` به کار میاد.
...
این کوئری، تعداد رکوردهای جدول tasks را به شما نشان می‌دهد. چیزی که در این خط برای شما آشنا است، همان دستور پرتکرار select است. عبارت جدیدی که اینجا می‌بینید، دستور count است؛ یک دستور پرکاربرد که هر چیزی را که به آن بگویید، می‌شمارد. اینجا هم می‌توانستید به جای *، اسم ستون‌های جدول خود را بنویسید.

همان‌طور که مشاهده می‌کنید، نام این ستون در خروجی count(*) است. با استفاده از دستور as، می‌توانید برای این ستون یک نام دلخواه انتخاب کنید:
```mysql
select
    count(*) as task_count
from tasks;
...
آیا کلیدواژه‌ی `where` را به یاد دارید؟ خوب، برای عمل شمارش نیز اگر بخواهید شرط خاصی تعیین کنید که فقط دسته‌ای از رکوردها شمارش شوند، می‌توانید این محدودیت را با `where` اعمال کنید.[^1]
...
به عنوان مثال، کوئری فوق تعداد همه‌ی قدم‌های تمام‌شده در جدول `steps` را به شما نشان می‌دهد. البته می‌توانید با دقت بیشتری نیز رکوردها را بشمارید. مثلاً بیایید ببینیم چند عدد از قدم‌های تسک ۱ به اتمام رسیده‌اند.
```mysql
select _count_(*) from steps
...
در این خط، به `count` ورودی `*` دادیم، در حالی که فقط با ستون `is_completed` کار داشتیم. لطفاً کوئری زیر را جایگزین کرده و آن را اجرا کنید.
```mysql
...
خروجی این خط نیز برابر با خروجی حالت قبلی است.
...
حالا این سؤال پیش می‌آید که آیا بین این دو کوئری تفاوتی وجود ندارد؟ بیایید برای روشن شدن موضوع، به سراغ ستون `task_list_id` برویم که می‌تواند مقدار `null` نیز داشته باشد. دو کوئری پایین را اجرا کنید و نتایج آنها را با یکدیگر مقایسه کنید.
```mysql
select _count_(*) from tasks;
...
مشاهده می‌کنید که نتیجه‌ی کوئری دوم دو واحد کمتر است. دلیل این امر آن است که در دو رکورد، ستون `task_list_id` مقدار `null` دارد. در واقع هنگامی که نام یکی از ستون‌ها را به `count` می‌دهید، رکوردهایی که آن ستون مقدار `null` دارند، شمارش نمی‌شوند. اما اگر `*` بدهید، `count` همه‌ی رکوردها را می‌شمارد، حتی آنهایی که در بعضی ستون‌ها مقدار `null` دارند. بنابراین، اگر ستونی که می‌شمارید `not null` باشد، این تفاوت دیگر معنای خاصی نخواهد داشت.
...
به شما گفته شده است که هزینه‌ی هر `task` نیز در پایگاه داده‌ی شما ثبت شود. برای این کار، ستون `cost` را به جدول تسک‌ها اضافه می‌کنیم:
```mysql
alter table tasks  
add column cost int;
```

پس از اجرای کوئری فوق، کوئری‌های زیر را نیز اجرا کنید. اگر درک آنها برای شما دشوار بود، نگران نباشید؛ زیرا هنوز دستوراتی که در این کوئری‌ها به کار رفته‌اند را یاد نگرفته‌اید و در آینده یاد خواهید گرفت. فعلاً کافی است بدانید که این خطوط به ستون `cost` رکوردهای قبلی جدول، مقدار می‌دهند تا مقدار آنها `null` نباشد.
```mysql
update tasks
set cost = 180
where id <= 2;
...
بیایید ببینیم تسک‌های کارمند شماره ۲ چقدر برای شرکت هزینه داشتند:
...
همانطور که مشاهده می‌کنید، کوئری مربوط به تابع `sum` نیز ساختاری مشابه با کوئری‌های `count` دارد. کوئری فوق مجموع هزینهٔ تسک‌های کارمند ۲ را محاسبه می‌کند.
...
خوب مشاهده می‌کنید که دیتاگریپ این اجازه را به شما نمی‌دهد. برخلاف `count`، تابع `sum` با `*` کار نمی‌کند. دلیل پیچیده‌ای نیز ندارد؛ تابع `sum` فقط ستون‌های عددی را به عنوان ورودی می‌پذیرد.

ورودی `sum` می‌تواند یک عبارت ریاضی نیز باشد؛ مثلاً فرض کنید که بخواهید دستمزد هر تسک را دو برابر حساب کنید اما نمی‌خواهد این عدد را در جدول تغییر دهید.
...
می‌خواهید میانگین هزینه‌ی تسک‌ها را حساب کنید تا ببینید شرکت شما چقدر برای تسک‌های مختلف هزینه می‌کند. برای این کار کافی است میانگین ستون `cost` تمام تسک‌ها را محاسبه کنید:
...
بعد از اینکه مجموع و میانگین هزینه‌ها را به دست آوردیم، می‌خواهیم بدانیم که کمترین و بیشترین `cost` در میان همه‌ی تسک‌ها چیست! SQL تابعی را برای شما فراهم کرده است که اگر فقط کمترین یا بیشترین مقدار یک ستون را بخواهید، می‌توانید از آنها استفاده کنید:
...
همان‌طور که مشاهده کردید، بر خلاف `avg` و `sum` که فقط ستون‌های عددی را پشتیبانی می‌کنند، توابع `min` و `max` برای ستون‌هایی از جنس رشته، تاریخ و هر نوع داده‌ی قابل مقایسه‌ی دیگر استفاده می‌شوند. (پیشتر نیز در جاوا یا برخی زبان‌های دیگر دیده‌اید که رشته‌ها بر اساس ترتیب الفبایی قابل مقایسه هستند و بسیاری از آبجکت‌ها یا نوع‌های دیگر نیز همین‌گونه هستند.)
توابع `min` و `max` هم مثل دو تابع قبلی، ورودی `*` رو قبول نمی‌کنن.

#### اگر فقط مقدار رو نخوام چی؟

کوئری‌ای که بالاتر دیدیم فقط عدد کمترین `cost` را نشان می‌دهد. اما ممکن است بخواهید نام تسکی که کمترین `cost` را دارد نیز بدانید. به نظر شما چگونه می‌توانیم این کار را انجام دهیم؟
...
### توابعی که دیدیم چی هستند؟

در این چند صفحه با ۵ تابع آشنا شدیم که شباهت‌های زیادی به یکدیگر داشتند. در زبان SQL، به این ۵ تابع و توابع مشابه آن‌ها `aggregate function` گفته می‌شود. این توابع روی چند سطر از جدول خود محاسبات یا عملیاتی را انجام می‌دهند و در نهایت دقیقاً یک مقدار را به‌عنوان خروجی برمی‌گردانند. همان‌طور که مشاهده کردیم، توابع aggregate اغلب با دستور `select` استفاده می‌شوند، اما در موارد دیگری مانند `having` نیز به کار می‌روند. پیشنهاد می‌کنم اگر تمایل دارید، درباره‌ی این موضوع بیشتر جستجو کنید.

## دستور `update`
 
 فرض کنیم شما دوست ندارید هیچ دو تسکی هزینه‌ی برابر داشته باشند. چگونه می‌توانیم مقدار هزینه (دستمزد) هر تسک را به‌روزرسانی کنیم؟

اگر به یاد داشته باشید، کمی پیش‌تر دستور `update` را برای اضافه کردن هزینه‌ها دیده‌اید. این دستور، مقدار رکوردها را تغییر می‌دهد. فقط کافی است نام جدول را به آن بدهید و بگویید که کدام ستون را چگونه می‌خواهید تغییر دهید.

بیایید با کوئری زیر کار رئیس را راه‌اندازی کنیم، سپس کمی بیشتر در مورد این دستور صحبت خواهیم کرد.:
...
اکنون اگر نگاهی به جدول خود بیندازیم، مشاهده می‌کنیم که هزینهٔ هر `task` با مقادیر دلخواه ما پر شده است.

![](images/db-2-13.png)

اگر در یکی از این به‌روزرسانی‌ها فراموش می‌کردیم که از `where` استفاده کنیم، چه اتفاقی می‌افتاد؟
...
این کوئری، هزینهٔ **همهٔ** تسک‌ها را برابر ۵۰۰ قرار می‌دهد. توجه داشته باشید که چه زمانی از چنین دستوری استفاده می‌کنید. به طور کلی، همیشه به دستوراتی مانند `delete` و `update` دقت کنید.
...
حالا اگر بخواهیم نزولی یا صعودی بودن خروجی را مشخص کنیم، چه؟

#### دستورهای `desc` و `asc`

پایگاه داده شما هوشمند است، کافی است ترتیب مورد نظر خود را به آن بدهید تا انجام دهد. بیایید برای نزولی کردن ترتیب رکوردهای خود، دستور زیر را اجرا کنیم:
...
اما هنوز یک مشکل وجود دارد: نمی‌دانیم رکوردهایی که `due_date` آن‌ها یکسان است، چگونه مرتب می‌شوند. پاسخ این سؤال بسیار ساده است؛ اگر ترتیب خاصی مشخص نکنیم، پایگاه داده آن رکوردها را به همان ترتیبی که اضافه شده‌اند، نمایش می‌دهد.

### multiple column ordering

حالا بیایید یک سناریو را در نظر بگیریم: می‌خواهیم میان تسک‌هایی که مهلت انجام (ددلاین) مشترک دارند، تسکی که بیشترین هزینه را دارد پیدا کنیم. برای این کار چه باید کرد؟
