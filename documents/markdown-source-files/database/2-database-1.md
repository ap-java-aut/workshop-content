...
حالا بهتر شد. الآن به جای این که تمام فیلدهای چند `step` مختلف در یک جدول زورچپان شده باشد، `step`ها جدول مستقل خودشان را دارند. کافی است به جدول tasks ستونی به اسم `task_list_id` اضافه کنید:
