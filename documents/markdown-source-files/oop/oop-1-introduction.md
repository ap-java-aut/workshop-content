...
در این داکیومنت و داکیومنت های بعدی، با شی‌گرایی آشنا می شویم. شی‌گرایی یا object-oriented programming (به اختصار OOP)،  طرز فکری است که برای تقسیم یک برنامهٔ بزرگ، به واحدهای کوچیک‌تر به اسم class. خیلی از چیزهایی که قبلا بهتون گفته بودیم «در آینده بهتون توضیح می‌دیم» را قرار است این‌جا بهتون توضیح بدیم!

پیشنهاد میکنیم در طول مطالعه این داکیومنت، با تدریس یارها در ارتباط باشید و اگر سوالی داشتید حتما از تدریس‌یارهاتون بپرسید، زیرا این داکیومنت یکی از مهم‌ترین داکیومنت‌های شماست و پایه ای برای اکثر مطالبی است که در ادامه ترم باهم مورد بحث قرار میدهیم مانند ارث‌بری، کپسوله‌سازی و مباحث مشابه .

## تایپ‌ها در جاوا

شما تا به حال با خیلی از type ها کار کردیپ و با ان‌ها آشنا شدید؛ مثل `int`، `float`، `String`، `JFrame`، `ArrayList` و ... هر برنامه‌نویسی توی جاوا، می‌تواند به راحتی تایپ‌های جدید برای خودش بسازد و به همین خاطر، احتمالا تا به حال میلیون‌ها تایپ توی جاوا نوشته شده. خود شما هم توی این داک قراره یاد بگیرید که چطور می‌تونید تایپ‌های جدید بسازید.

همون‌طور که توی جلسهٔ صفر دیدید، تایپ‌های `int`، `float`، `double`، `long` و `char`، به اسم «تایپ‌های اولیه» یا primitive types شناخته می‌شوند. فهرست کامل ان‌ها به شکل زیره:
...
به این تایپ‌ها، مانند تیکه‌های کوچیک لگو نگاه کنید که تایپ‌های پیچیده‌تر مانند `String`، `JFrame`، `ArrayList` و هر تایپ دیگری را تشکیل می‌دهند. به این تایپ‌های پیچیده‌تر، reference type می‌گویند.در ادامه یک مثال از انها را مورد بررسی قرار میدهیم. کد زیر را توی IntelliJ کپی کنید:
...
موس را روی `String` ببرید و کلیک راست کنید. از صفحه‌ای که باز می‌شود، منوی Go To و بعد از ان، Declaration or Usagesارو انتخاب کنید:

![](images/oop-1-1.png)

صفحهٔ جدیدی براتون باز می‌شود که شامل پیاده‌سازی تایپ `String `است:

![](images/oop-1-2.png)

شما احتمالا هنوز کامل این کد را نمی‌فهمید، ولی توی این کد، نشون داده شده است که تایپ `String` از کنار هم گذاشتن چه تایپ‌های دیگه‌ای درست شده است. مثلا، توی خط ۱۶۰ همین تصویر می‌بینید که `String`، توی خودش یک آرایه از `byte` ها داره که اسمش `value` ‍است. پایین‌تر می‌بینید که در خودش، یک `int` به اسم `hash` داره. اگر دوست دارید، کمی مقدار توی این کد بالا و پایین بشید، خیلی از تیکه‌های ان براتون غریبه‌هستند و البته خیلی از بخش‌ها هم برای شما آشنا به نظر میاد.

## کلاس‌ها

کلاس‌ها، به شما اجازه می‌دهند تا `type` های جدیدی درست کنید. بهترین راه یادگیری انا، این است که کمی  باهاشون کار کنید، پس بیاید تا با هم یه کلاس جدید به اسم `Student` درست کنیم. کد زیر را توی IntelliJ بنویسید:
...
- متغیر `grade`: متغیری از جنس `double` که معدل دانشجو را نشون می‌دهد.
- متغیر `name`: متغیرِ `String` ای که اسم دانشجو را نشون می‌دهد.
- متغیر `studentID`: متغیر `String` ای که شماره دانشجویی شخص را نشون می‌دهد.

به  متغیرهایی که یک کلاس را تشکیل می‌دهند، `field` های اون کلاس می‌گویند. نگران کلیدواژهٔ `public`نباشید، در جلسه های اینده بهتون مفهوم اون را توضیح می‌دیم ولی برای الآن، لازمه که اون را قبل از همهٔ `field` های کلاس‌هاتون بذارید. حالا، با کلیدواژهٔ `new`، یک دانشجوی جدید به اسم قلی بسازید و فیلدهای ان را مقداردهی کنید:
...
می‌بینید که مشخصات قلی، به درستی روی صفحه چاپ می‌شه. تبریک! شما اولین type خودتون را ساختید و یه متغیر هم از جنس اون درست کردید. بیاید به این type جدید، چیزهای بیشتری اضافه کنیم. به `Student`، فیلدی به اسم `grades` از جنس `ArrayList<Double>` رو اضافه کنید. این فیلد، نمرات دانشجو توی درس‌های مختلف رو نشون می‌ده:
...
بعد از این کار، چندتا نمرهٔ رندوم به قلی بدهید:
...
حالا، جایی که دارید مشخصات قلی را چاپ می‌کنید، این نمرات هم چاپ کنید:
...
کدتون را اجرا کنید. خروجی کدتون باید به این شکل باشد:
...
جایی از کدتون که دارین مشخصات `gholi` را چاپ می‌کنید، خطوط زیر را اضافه کنید:
...
کدتون را دوباره اجرا کنید. خروجی‌ای مثل زیر می‌بینید:
...
یه لحظه به خود تابع `main` توجه کنید. می‌بینید که خود ان هم توی کلاسی به اسم `Main`‍است! شما توی تمام این مدت، داشتید کلاس `Main` را تعریف می‌کردید بدون این که حتی خبر داشته باشید:
...
البته که متغیر ساختن از جنس `Main` خیلی کار خوبی نیست! ولی جالبه که تا همین الآن هم شما از `class` ها استفاده می‌کردید، بدون این که بدونید. =)))

 در ادامه، کمی رسمی‌تر و قدم به قدم‌تر کلاس‌ها را بررسی می کنیم.

### تعریف کلاس

تعریف کلاس، کار سختی نیست، فقط از کلیدواژهٔ `class` استفاده کنید و اسم کلاستون را بنویسید:
...
هر چیزی که بین دو براکت می اید، متعلق به ان کلاس است. کلاس‌ها، به شما اجازه می‌دهند که کدهای خیلی خیلی بزرگ را، به تیکه‌های کوچیک‌تر تقسیم کنید و با این کار، برنامه‌های منظم‌تر و بهتری داشته باشید.

یادآوری می‌کنیم که type هایی که به وسیلهٔ `class` ها تعریف می‌شود، همگی از reference typeها محسوب می‌شوند.

## ایجاد Object

به متغیرهایی که از جنس یک reference type درست می‌شوند، object یا instance می‌گویند.در کد زیر، قلی و ممد و سلطان همگی object هایی از جنس `Student` ان:
...
تلاش کنید این کد را اجرا کنید. می‌بینید که طبیعتا، بدون هیچ مشکلی اجرا می‌شود. شما توی این کد سه object جدید تعریف کردید و هیچ‌جایی ان‌ها را مقداردهی نکردید، و جایی هم ازشون استفاده نشده.

برای این که اون‌ها را مقداردهی کنید، از کلید‌واژهٔ new استفاده کنید:
...
```
همانطور که میبینید در این کد ما به اجبار کد های تکراری زیادی زده ایم. کدی که مربوط به بخش مقداردهی field های یگ object
است سه بار تکرار شده پس بهتر است این کدها را به یک متد تبدیل کنیم. `Constructor`  ها، متدهایی هستن که دقیقا برای همین کار، به کمک ما می ایند
...
می‌بینید که این متد، کمی  ظاهر متفاوتی دارد!  نوع خروجی‌ش مشخص نشده،  جایی از کلیدواژهٔ `return` استفاده نشده. به این متد، `Constructor` می‌گویند و وظیفهٔ ان، مقداردهی یک object جدید از کلاس `Student` ‍است. در constructor بالا، ما برای ساخت یه `Student` جدید، سن، اسم و شماره دانشجویی‌ش را ورودی گرفته ایم و با استفاده از ان ها فیلدهای `Student` را برای object جدیدمون مقداردهی کردیم.
...
تا قبل از این که ما برای کلاس `Student` کانستراکتور تعریف کرده باشیم، می‌تونستیم هنوز با استفاده از کلیدواژهٔ `new` از ان object بسازیم:
...
وقتی یک کلاس  `constructor` نداشته باشد، جاوا خود یک `constructor`  خالی برایش می‌نویسد. این  `constructor`، به شکل زیر است:

```java
public Student() {}
```

توی این کانستراکتور هیچ اتفاقی نمی‌افتد و هیچ field ای مقداردهی نمی‌شود. صرفاً اضافه تا شما بتونید به راحتی با استفاده از `new Student()`، آبجکت‌های جدید از این کلاس بسازید. به محض این که شما اولین `constructor` کلاس `Student` ر بنویسید، دیگه نمی‌تونید از default constructor جاوا استفاده کنید. توی کد فعلی‌تون این خط را بنویسید و کد را اجرا کنید:
...
از اون‌جایی که کلاس `student`، الآن یک `constructor` برای خودش دارد، جاوا برای شما کانستراکتور دیفالت را درست نمی‌کند.

### کلاس‌ها با چند کانستراکتور

شما می‌تونید برای رفع مشکلی که بالاتر به اون برخورد کردید، خودتون یه کانستراکتور خالی برای `Student` بنویسید:

```java
public Student() {}
```

دقت کنید که لازم نیست کانستراکتور قبلی‌تون را پاک کنید! هر کلاس، می‌تواند چندین constructor داشته باشد. شما می‌توانید این constructor را در کنار کانستراکتور قبلی‌تون بنویسید و کدتون همچنان کار می‌کند.

## field ها

متغیر های یک کلاس، یا fieldها، ویژگی‌های اون کلاس را نشون می‌دهند. شما تا الآن تعداد خوبی field برای کلاس student تعریف کردید و با اون‌ها آشنا شدید:
...
شما توی یک کلاس، حتی می‌تونید field هایی از جنس همون کلاس تعریف کنید! مثلاً، می‌تونید فیلد friend را برای `Student`، از جنس خود `Student` تعریف کنید:
...
برای الآن، بدون این که فیلدهای شهرام را مقداردهی کنید، سعی کنید تا اطلاعات اون را چاپ کنید:
...
کدتون را اجرا کنید، خروجی زیر را برای شهرام می‌بینید:
...
می‌بینید که علی رغم این که شما به شهرام اسم و سن و شماره دانشجویی ندادید، خود جاوا یه سری مقدار به اون‌ها داده. مقدار دیفالت جاوا برای فیلدهایی که مقداردهی نشده اند، به این شکل است:
- **متغیرهای عددی (مثل `int`، `float` و امثال اون‌ها):** مقدار `0` را به خودشون می‌گیرند.
- **متغیرهای `char`:** مقدار `'\0'` را به خودشون می‌گیرند.
- **متغیرهای `boolean`:** مقدار `false` به خودشون می‌گیرند.
- **متغیرهای از جنس reference type :** مقدار `null` به خودشون می‌گیرند.

کلیدواژه `null`، یکی از کلیدواژه‌های خاص جاواست که نشون‌دهندهٔ اینه که یک متغیر از جنس reference type، هنوز مقداری به خودش نگرفته است. اگر توی کد قبلی‌مون، خط زیر را بنویسیم:
...
بعضی ویژگی‌ها، متعلق به هیچ object ای نیستند، ولی بی‌ربط به خود `class` هم نیستند. مثلا توی کلاس  `Student`   ویژگیِ «تعداد کل دانشجوها» متعلق به هیچ کدوم از قلی، ممد یا سلطان نیست، ولی به کلاس  `Student`  ربط داره.

به این ویژگی‌ها، ویژگی‌های `static`می‌گیم. اون‌ها به خود class مرتبط اند و بین تمام instanceهای اون  class مشترکن. فیلد`static` زیر را برای دانشجوها تعریف کنید:
...
سپس توی همهٔ constructor هایی که برای `Student` نوشتین، به مقدار اون یکی اضافه کنید. با این کار، با ساخت هر دانشجو، تعداد کل دانشجوها یکی زیاد می‌شود:
...
حالا، کد زیر را توی main بنویسید:
...
می‌بینید که ما برای دسترسی به `totalNumberOfStudents`، از خود کلاس `Student` استفاده کردیم. می‌تونستید با کد زیر، از هر کدوم از instance های `student` هم به اون دسترسی پیدا کنید، ولی کار چندان خوبی نیست:
...
تا اینجای کار، با کلاس‌های نسبتاً ساده ای سر و کار داشتید. اما جاهای مختلف به «رفتار کلاس» یا این ایده که کلاس ما کاری انجام بده اشاره کردیم، اینجاست که متدها وارد عمل میشوند: به طور کلی زمانی که بخواهید توی کدتون تصمیمی بگیرید یا عملیات منطقی انجام بدید یا کلا کاری انجام بدید، باید از متدها استفاده کنید. متدها انقدر مهمن که حتی توی اولین مواجه‌­تون با جاوا از متد `main` استفاده کردید و توی اون کدتون را نوشتید! توی این بخش قراره دقیق­تر و کامل­تر با متدها آشنا بشید. کد زیر، یک مثال ساده از یک متده:
...
توی این مثال، یه کلاس `Refrigerator` داریم که یک فیلد از نوع `int` دارد به نام `numberOfBananas` و یک متد دارد که نوع خروجیش `void` ‍ه (خروجی ندارد) و یک ورودی (argument) از نوع `int` دارد. با استفاده از این متد می­توانید از توی یخچالتون موز بردارید! حالا خودتون یه متد اضافه کنید که باهاش بتونید توی یخچالتون موز بذارید. متدتون احتمالاً چیزی شبیه به این میشود:
...
هر تایپ دیگری در جاوا، reference typeاست . `String`، `JFrame`، `ArrayList` و حتی تایپ‌هایی مثل `Car` و `Student` که تا این‌جای کار تعریف کردیم، همگی reference type اند. هر reference type ای با یک کلاس تعریف شده است.
...
شاید بتونید حدس بزنید که این‌جا چه اتفاقی افتاد. با ورودی دادن چنگیز به `resetID`، در واقع شما اشاره‌گرتون به heapرا به این تابع ورودی دادید. پس متغیر `student` در `resetID` و `changiz` و `bangiz`، هر سه به یک نقطه از heap اشاره می‌کنند و مثل قبل، با تغییر فیلدهای یکی از ان‌ها، هر سه تغییر می‌کنند. به این نوع ورودی دادن به توابع، اصطلاحا passing by referenceمی‌گویند.
...
- این که چگونه می‌توان با استفاده از `class`، **typeهای جدید** تعریف کرد
...
- تفاوت بین **متغیرهای محلی** و **fieldها**
...
- درک بهتر از نحوهٔ کار **referenceها** و تاثیر آن‌ها در تغییر داده‌ها
