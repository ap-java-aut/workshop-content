...
در این داکیومنت به مفاهیم پیشرفته‌تری از شیءگرایی می‌پردازیم. چندریختی (Polymorphism) یکی از این مفاهیم است که این امکان را می‌دهد که متدهایی با نام‌های یکسان را به شکل‌های متفاوت پیاده‌سازی کنیم. اینترفیس نیز به کلاس‌های متفاوت، این امکان را می‌دهد که متدهای مشترکی را بدون به اشتراک گذاشتن کد و جزئیات پیاده‌سازی، داشته باشند. اینترفیس ها به ما کمک می‌کنند برنامه ساختارمندتر و منعطف‌تری داشته باشیم.

حتماً کدهای این داک را اجرا کنید و هر سوالی که داشتید از تدریس‌یارها بپرسید، در مورد آن سرچ کنید و از DeepSeek راجع‌بهش اطلاعات بخواهید.

## Polymorphism

در دنیای واقعی وقتی اطرافتان را نگاه می‌کنید، می‌بینید که خیلی از کارهای یکسان در موجودات مختلف به شکل‌های متفاوتی انجام می‌شود. مثلاً راه رفتن را در نظر بگیرید؛ هم اسب، هم انسان هر دو راه می روند، اما میدانیم که در انجام ان بسیار متفاوت‌اند یا مثلاً غذا خوردن، حرف زدن و خصوصیات دیگری که در موجودات مشترک‌اند اما در جزئیات و عملکرد متفاوت‌اند.
...
- **Compile-time Polymorphism (Method Overloading)**:
  ...
  در این حالت چند متد با یک اسم ولی با ورودی‌های متفاوت در یک کلاس تعریف می‌کنید. این کار به شما اجازه می‌دهد که یک کار مشابه را با ورودی‌های مختلف انجام بدهید.

- **Run-time Polymorphism (Method Overriding)**:
  
 این یعنی یک متد در کلاس والد تعریف شده است و بعد در کلاس فرزند با همان اسم ولی پیاده‌سازی متفاوت بازنویسی می‌شود. این کار به شما اجازه می‌ده رفتار یه متد را در کلاس‌های فرزند تغییر بدهید.
...
مثلا در کد زیر، متد `makeSound` در `Dog` دوباره تعریف شده است. لازم است که وقتی متدی را override می‌کنید، حتما بالای آن از `@Override` استفاده کنید، وگرنه IntelliJ به شما هشدار می‌دهد.
...
کلاس‌ها، سوپرکلاس‌های عادی یا abstract را extend میکردند و ویژگی‌های آنها را به ارث میبردند، ولی interface ها را implement (پیاده‌سازی) میکنند. Implement کردن یک اینترفیس به این معناست که کلاسی که میخواهد interface مورد نظر را پیاده‌سازی کند، قول میدهد که بدنه همه متدهای تعریف شده در اینترفیس را پیاده‌سازی کند (در غیر این صورت به کامپایل ارور میخورد).
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| اگر فقط به یک لایه وراثت نیاز دارید و مشخص است که کلاس‌های فرزند شباهت‌های زیادی در پیاده‌سازی بین خودشون دارن، یک کلاس ابسترکت می‌تواند مناسب‌تر باشد.                                                                                                            | اگر نیاز دارید یک کلاس از چند منبع مختلف ویژگی‌هایی دریافت کند، اینترفیس‌ها گزینه بهتری هستند.                                                                |
| اگر برخی متدها باید در همه‌ی کلاس‌های فرزند یکسان باشند و نمی‌خواهید کد تکراری بنویسید، کلاس ابسترکت بهتر است.                                                                                                                                                       | اگر فقط می‌خواهید یک قرارداد تعیین کنید که همه‌ی کلاس‌ها موظف باشند متدهایی را پیاده‌سازی کنند و لزوما این پیاده‌سازی‌ها شبیه هم نیستن، اینترفیس گزینه‌ی بهتری است. |
| اگر کلاس باید مقادیر پیش‌فرض یا داده‌ای را در خود نگه دارد، کلاس ابسترکت بهتر است.<br><br>مزیت کلاس‌های ابسترکت این است که میتوانیم هم متغیرهای مشترک و هم متدهای مشترک با سوپرکلاس را تعریف کنیم و در صورت نیاز به تغییر پیاده سازی در یک کلاس فرزند، override کنیم. | اگر فقط متدهای مورد نیاز را تعریف می‌کنید و هیچ داده‌ و فیلدی در کلاس پایه نیاز ندارید(نیاز به متغیرهای نمونه ندارید) اینترفیس بهترین انتخاب است.                |
...
جاوا با این خطا به شما می‌گوید که تبدیل متغیرهای `long` به `int`، خطرناک است و می‌تواند منجر به خطاهای overflow شود. مثلاً در همین مثال، عدد `10000000000` به قدری بزرگ است که در یک متغیر `int` جا نمی‌شود و به همین خاطر تبدیل آن به یک `int` مشکلاتی برای شما به همراه دارد.
...
- این‌که **interface** چیست و برای چه به آن نیاز داریم
- مقایسۀ **interface ها** و **abstract class ها** و اینکه چرا نمی‌توان به‌صورت مستقیم از آنها شیء ساخت
...
- کلاس **object** چیست
- متدهای ناآشنایی که توسط ما تعریف نشده اند اما قابل استفاده اند، چه ارتباطی با کلاس **Object** دارند
