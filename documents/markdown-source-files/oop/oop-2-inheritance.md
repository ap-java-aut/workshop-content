...
در این داکیومنت، علاوه بر ارث‌بری، به چند مفهوم دیگر جاوا مثل lambda ها، inner class ها و annotation هم می‌پردازیم.

حتماً کدهای این داک رو اجرا کنید و هر سوالی که داشتید از تدریس‌یارها بپرسید، در موردش سرچ کنید و از ChatGPT راجع بهش اطلاعات بخواید. ارث‌بری از مفاهیم مهم و جالب OOP است که یادگیری آن اهمیت زیادی دارد.
...
بعد از اجرای کدتون، باید چنین چیزی ببینید:
...
حالا که کدتون کار می‌کنه، بیاید راجع به شباهت‌های این دو کلاس صحبت کنیم. ظاهر این کلاس‌ها خیلی شبیه به هم‌دیگه است. هر دو فیلدهایی به اسم `brand` و `speed` دارند، با متدی به اسم `accelerate` سرعت وسیلهٔ نقلیه را زیاد می‌کنند و همینطور متدهایی دارند که اطلاعات آن‌ها رو نمایش می‌دهند. در واقع، اگر یک بار به کدتون نگاه کنید، می‌بینید کلاس‌های `Bike` و `Car` تفاوت چندانی ندارند! حالا بیاید یک کلاس جدید مثل `MotorCycle` در فایل جدیدی به اسم `MotorCycle.java` بنویسیم:
...
کلاس جدیدی که ساختیم هم کار می‌کنه، ولی باز هم خیلی شبیه‌ به دو کلاس قبلیه! به نظر میاد که ما هر وسیلهٔ نقلیه‌ای که بخواهیم اضافه کنیم، باید کدهای کلاس‌های قبلی را تکرار کنیم. همانطور که در داک clean code خوندید، ما در برنامه‌نویسی، سعی می‌کنیم از کدهای تکراری دوری کنیم.

### راه‌حل

راه حل این مشکل این است که **ویژگی‌های مشترک تمام این کلاس‌ها رو، داخل کلاس جدیدی به اسم `Vehicle` بیاریم و بعد، با استفاده از ارث‌بری، اون ویژگی‌ها رو به همه این کلاس‌ها منتقل کنیم**. حالا این‌ که «ویژگی‌های یک کلاس‌ رو به کلاسی دیگه منتقل کنیم» یعنی چه؟ بیاید تا با کد زدن ببینیم. توی فایل `Vehicle.java`، کلاس `Vehicle` رو ایجاد کنید. این کلاس، به جای این که نمایندهٔ ماشین و موتور و دوچرخه باشه، نمایندهٔ یک «وسیلهٔ نقلیه‌»ست و ویژگی‌های مشترک تمام کلاس‌های قبل رو داخل خودش نگه داشته:
...
می‌بینید که فیلدها و متدهای مشترک تمام وسایل نقلیه، یعنی `brand`، `speed`، `accelerate` و غیره، همگی داخل این کلاس هستن. حالا بیاید کد کلاس `Car` رو عوض کنیم:
...
در این کد، شما برای اولین بار با کلیدواژهٔ `extends` آشنا می‌شین. این کلیدواژه در جاوا برای تعیین ارث‌بری استفاده می‌شه. در کد بالا کلاس `Car`، تمام فیلدها و متدهای کلاس `Vehicle` رو داره. مثلا در خط اول متد `showCarInfo()`، می‌بینید که متد `showInfo()` صدا زده شده اما `Car` چنین متدی نداره. در واقع، این متد در کلاس `Vehicle` تعریف شده و چون `Car` از `Vehicle` ارث‌بری کرده، پس می‌تونه از متدها و فیلدهای اون استفاده بکنه!

حالا در `main` کد زیر رو اجرا کنید تا مطمئن بشید همه چیز درست کار می‌کنه:
...
می‌بینید که حتی تونستیم از متد `accelerate` هم برای `myCar` استفاده کنیم، در حالی که این متد اصلا در `Car` تعریف نشده! این موضوع فقط به خاطر اینه که `Car` از `Vehicle` ارث‌بری می‌کنه، و به همین دلیل به تمام فیلدها و متدهای اون دسترسی داره.

حالا کد‌ کلاس‌های `Bike` و `MotorCycle` رو به شکل زیر تغییر بدید تا اون‌ها هم از کلاس `Vehicle` ارث‌بری کنن:
...
می‌بینید که حجم کدهای تکراری خیلی کمتر شده. با کد زیر، این کلاس‌های جدید رو تست کنید:
...
در کد بالا، به کلاس `Vehicle` که از اون ارث‌بری شده superclass یا کلاس پدر می‌گن. همچنین به کلاس‌های `Car`، `Bike` و `MotorCycle` که از کلاس `Vehicle` ارث‌بری کردن subclass یا کلاس‌های فرزند می‌گن. هر کلاس، از حداکثر یک کلاس می‌تونه به صورت مستقیم ارث‌بری کنه، یعنی هر کلاس حداکثر یک پدر داره.

همینطور که می‌بینید در خط اول constructor های subclass ها، از عبارتی به اسم `super` استفاده شده:
...
هر زمان از کلید واژه جدید super استفاده می‌کنیم، یعنی قصد داریم از فیلدها، متدها یا کانستراکتورهای کلاس پدر استفاده کنیم. مثلا در این‌جا، با استفاده از کلیدواژهٔ `super`، نشون دادیم که می‌خوایم کانستراکتور زیر از کلاس پدر رو استفاده کنیم تا فیلدهای `brand` و `speed` برای آبجکت جدید مقداردهی بشن:
...
یا مثلا، در متد `showBikeInfo()`، می‌تونیم با استفاده از `super`، دقیق‌تر مشخص کنیم که متد `showInfo` ی کلاس `Vehicle` صدا زده بشه:
...
البته نتیجه کپی کردن کد بالا به جای کد `ShowBikeInfo()` اینه که سرعت و برند دوچرخه‌ها دو بار چاپ بشن! هدف از نشون دادن کد بالا این بود که ببینید با کلیدواژهٔ super، به تمام ویژگی‌های کلاس پدر دسترسی خواهید داشت. در ادامه این داک، متدهای کلاس پدر رو Override می‌کنید و بیشتر به اهمیت این موضوع پی می‌برید.
