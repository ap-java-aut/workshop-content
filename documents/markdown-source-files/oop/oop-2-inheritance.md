...
حالا بیاید راجع به شباهت‌های این دو کلاس صحبت کنیم. ظاهر این کلاس‌ها خیلی شبیه به هم‌دیگه است. هر دو فیلدهایی به اسم `brand` و `speed` دارند، با متدی به اسم `accelerate` سرعت وسیلهٔ نقلیه را زیاد می‌کنند و همینطور متدهایی دارند که اطلاعات آن‌ها را نمایش می‌دهند. در واقع، اگر یک بار به کدتون نگاه کنید، می‌بینید کلاس‌های `Bike` و `Car` تفاوت چندانی ندارند! حالا بیاید یک کلاس جدید مثل `MotorCycle` در فایل جدیدی به اسم `MotorCycle.java` بنویسیم:
...
راه حل این مشکل این است که **ویژگی‌های مشترک تمام این کلاس‌ها را، داخل کلاس جدیدی به اسم `Vehicle` بیاریم و بعد، با استفاده از ارث‌بری، آن ویژگی‌ها را به همه این کلاس‌ها منتقل کنیم**. حالا این‌ که «ویژگی‌های یک کلاس‌ را به کلاسی دیگر منتقل کنیم» یعنی چه؟ بیاید تا با کد زدن ببینیم. توی فایل `Vehicle.java`، کلاس `Vehicle` رو ایجاد کنید. این کلاس، به جای این که نمایندهٔ ماشین و موتور و دوچرخه باشد، نمایندهٔ یک «وسیلهٔ نقلیه‌» است و ویژگی‌های مشترک تمام کلاس‌های قبل را داخل خود نگه می‌دارد:
...
در این کد، شما برای اولین بار با کلیدواژهٔ `extends` آشنا می‌شید. این کلیدواژه در جاوا برای تعیین ارث‌بری استفاده می‌شود. در کد بالا کلاس `Car`، تمام فیلدها و متدهای کلاس `Vehicle` رو دارد. مثلا در خط اول متد `showCarInfo()`، می‌بینید که متد `showInfo()` صدا زده شده اما `Car` چنین متدی ندارد. در واقع، این متد در کلاس `Vehicle` تعریف شده و چون `Car` از `Vehicle` ارث‌بری کرده، پس می‌تواند از متدها و فیلدهای آن استفاده بکند!
...
در کد بالا، به کلاس `Vehicle` که از آن ارث‌بری شده superclass یا کلاس پدر می‌گویند. همچنین به کلاس‌های `Car`، `Bike` و `MotorCycle` که از کلاس `Vehicle` ارث‌بری کردند subclass یا کلاس‌های فرزند می‌گویند. هر کلاس می‌تواند از حداکثر یک کلاس به صورت مستقیم ارث‌بری کند. یعنی هر کلاس حداکثر یک پدر دارد.
