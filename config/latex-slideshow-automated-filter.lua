...
function Header(el)
  ...
  if level == 1 then
    -- Level 1: Section (no frame, just section)
    current_section = title
    return pandoc.RawBlock('latex', '\\section{' .. title .. '}')
  elseif level == 2 then
    -- Level 2: Subsection + new frame
    current_subsection = title
    return pandoc.RawBlock('latex', '\\subsection{' .. title .. '}\n\\begin{frame}{' .. title .. '}')
  elseif level == 3 then
    -- Level 3: New frame with title
    return pandoc.RawBlock('latex', '\\end{frame}\n\n\\begin{frame}{' .. title .. '}')
  else
...
function Para(el)
  if #el.content == 1 and el.content[1].t == "Image" then
    ...
    -- For slides, use smaller width to fit better
...
-- Add a final frame closing at the end of document
function Pandoc(doc)
  -- Add closing frame at the end
